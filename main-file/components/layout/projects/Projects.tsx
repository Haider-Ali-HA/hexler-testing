...
const Projects = () => {
    ...
    //Getting Categories
    const categories = ['All', ...Array.from(new Map(projectsData.flatMap(item => item.categories).map(category => [category, true])).keys())];

    //Filtering projects based on the selected category
    const filteredProjects = selectedCategory === 'All'
        ? projectsData
        : projectsData.filter(item => item.categories.includes(selectedCategory));
                    ...
                    <div className="project-listings">
                        {filteredProjects.map((item, index) => (
                            <Link href={item.path} target='_blank' key={index} className="project-card">
                                <img
                                    src={item.image || 'https://plus.unsplash.com/premium_photo-1675793715030-0584c8ec4a13?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'}
                                    alt={item.name}
